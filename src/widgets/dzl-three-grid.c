...
G_DEFINE_TYPE_WITH_PRIVATE (DzlThreeGrid, dzl_three_grid, GTK_TYPE_CONTAINER)
...
static DzlThreeGridChild dummy;

static DzlThreeGridChild *
dzl_three_grid_child_new (void)
{
  return g_slice_new0 (DzlThreeGridChild);
}

static void
dzl_three_grid_child_free (gpointer data)
{
  DzlThreeGridChild *child = data;

  g_clear_object (&child->widget);
  g_slice_free (DzlThreeGridChild, child);
}

static DzlThreeGridChild *
dzl_three_grid_find_child (DzlThreeGrid *self,
                           GtkWidget    *widget)
{
  ...
  for (guint i = 0; i < priv->children->len; i++)
    {
      DzlThreeGridChild *child = g_ptr_array_index (priv->children, i);
...
static void
dzl_three_grid_add (GtkContainer *container,
                    GtkWidget    *widget)
{
  DzlThreeGrid *self = (DzlThreeGrid *)container;
  DzlThreeGridPrivate *priv = dzl_three_grid_get_instance_private (self);
  DzlThreeGridChild *child;

  g_assert (DZL_IS_THREE_GRID (self));
  g_assert (GTK_IS_WIDGET (widget));

  child = dzl_three_grid_child_new ();
...
static void
dzl_three_grid_remove (GtkContainer *container,
                       GtkWidget    *widget)
{
  DzlThreeGrid *self = (DzlThreeGrid *)container;
  DzlThreeGridPrivate *priv = dzl_three_grid_get_instance_private (self);
...
dzl_three_grid_get_request_mode (GtkWidget *widget)
{
  return GTK_SIZE_REQUEST_HEIGHT_FOR_WIDTH;
}

static void
dzl_three_grid_get_column_width (DzlThreeGrid       *self,
                                 DzlThreeGridColumn  column,
                                 gint               *min_width,
                                 gint               *nat_width)
{
  ...
  g_assert (DZL_IS_THREE_GRID (self));
  g_assert (column >= DZL_THREE_GRID_COLUMN_LEFT);
  g_assert (column <= DZL_THREE_GRID_COLUMN_RIGHT);
...
static void
dzl_three_grid_get_preferred_width (GtkWidget *widget,
                                    gint      *min_width,
                                    gint      *nat_width)
{
...
static void
row_info_merge (DzlThreeGridRowInfo       *row_info,
                const DzlThreeGridRowInfo *other)
{
  g_assert (row_info);
  g_assert (other);
                 ...
                 DzlThreeGridChild *child)
{
  ...
  if (row_info == NULL)
    {
      row_info = g_new0 (DzlThreeGridRowInfo, 1);
...
static void
dzl_three_grid_get_preferred_height_for_width (GtkWidget *widget,
                                               gint       width,
                                               gint      *min_height,
                                               gint      *nat_height)
{
...
sort_by_row (gconstpointer a,
             gconstpointer b)
{
  const DzlThreeGridRowInfo *info_a = a;
  const DzlThreeGridRowInfo *info_b = b;

  return info_a->row - info_b->row;
}

static void
dzl_three_grid_size_allocate_children (DzlThreeGrid       *self,
                                       DzlThreeGridColumn  column,
                                       gint                row,
...
static void
dzl_three_grid_size_allocate (GtkWidget     *widget,
                              GtkAllocation *allocation)
{
  DzlThreeGrid *self = (DzlThreeGrid *)widget;
  DzlThreeGridPrivate *priv = dzl_three_grid_get_instance_private (self);
  ...
  for (iter = values, i = 0; iter != NULL; iter = iter->next, i++)
    {
      DzlThreeGridRowInfo *row_info = iter->data;
  ...
  for (i = 0; i < n_rows; i++)
    {
      ...
      DzlThreeGridRowInfo *row_info = size->data;
      ...
      if (dir == GTK_TEXT_DIR_LTR)
        dzl_three_grid_size_allocate_children (self, DZL_THREE_GRID_COLUMN_LEFT, row_info->row, &child_alloc, baseline);
      else
        dzl_three_grid_size_allocate_children (self, DZL_THREE_GRID_COLUMN_RIGHT, row_info->row, &child_alloc, baseline);

      child_alloc.x = area.x + left + priv->column_spacing;
      child_alloc.width = center;
      child_alloc.y = area.y;
      child_alloc.height = size->minimum_size;
      dzl_three_grid_size_allocate_children (self, DZL_THREE_GRID_COLUMN_CENTER, row_info->row, &child_alloc, baseline);
...
static void
dzl_three_grid_forall (GtkContainer *container,
                       ...
                       gpointer      user_data)
{
  ...
  for (guint i = priv->children->len; i > 0; i--)
    {
      DzlThreeGridChild *child = g_ptr_array_index (priv->children, i - 1);
...
static void
dzl_three_grid_get_child_property (GtkContainer *container,
                                   GtkWidget    *widget,
                                   guint         prop_id,
                                   GValue       *value,
                                   GParamSpec   *pspec)
{
  DzlThreeGrid *self = (DzlThreeGrid *)container;
  DzlThreeGridChild *child = dzl_three_grid_find_child (self, widget);
...
static void
dzl_three_grid_set_child_property (GtkContainer *container,
                                   GtkWidget    *widget,
                                   guint         prop_id,
                                   const GValue *value,
                                   GParamSpec   *pspec)
...
static void
dzl_three_grid_finalize (GObject *object)
{
...
static void
dzl_three_grid_get_property (GObject    *object,
                             guint       prop_id,
                             GValue     *value,
                             GParamSpec *pspec)
{
  DzlThreeGrid *self = DZL_THREE_GRID (object);
  DzlThreeGridPrivate *priv = dzl_three_grid_get_instance_private (self);
...
static void
dzl_three_grid_set_property (GObject      *object,
                             guint         prop_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
...
static void
dzl_three_grid_class_init (DzlThreeGridClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);
  GtkContainerClass *container_class = GTK_CONTAINER_CLASS (klass);

  object_class->finalize = dzl_three_grid_finalize;
  object_class->get_property = dzl_three_grid_get_property;
  object_class->set_property = dzl_three_grid_set_property;

  widget_class->get_request_mode = dzl_three_grid_get_request_mode;
  widget_class->get_preferred_height_for_width = dzl_three_grid_get_preferred_height_for_width;
  widget_class->get_preferred_width = dzl_three_grid_get_preferred_width;
  widget_class->size_allocate = dzl_three_grid_size_allocate;

  container_class->add = dzl_three_grid_add;
  container_class->forall = dzl_three_grid_forall;
  container_class->get_child_property = dzl_three_grid_get_child_property;
  container_class->remove = dzl_three_grid_remove;
  container_class->set_child_property = dzl_three_grid_set_child_property;
...
static void
dzl_three_grid_init (DzlThreeGrid *self)
{
  DzlThreeGridPrivate *priv = dzl_three_grid_get_instance_private (self);

  priv->children = g_ptr_array_new_with_free_func (dzl_three_grid_child_free);
...
GType
dzl_three_grid_column_get_type (void)
{
  ...
  if (g_once_init_enter (&type_id))
    {
      ...
      static const GEnumValue values[] = {
        { DZL_THREE_GRID_COLUMN_LEFT, "DZL_THREE_GRID_COLUMN_LEFT", "left" },
        { DZL_THREE_GRID_COLUMN_CENTER, "DZL_THREE_GRID_COLUMN_CENTER", "center" },
        { DZL_THREE_GRID_COLUMN_RIGHT, "DZL_THREE_GRID_COLUMN_RIGHT", "right" },
        { 0 }
      };
      _type_id = g_enum_register_static ("DzlThreeGridColumn", values);

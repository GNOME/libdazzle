...
G_DEFINE_TYPE (DzlBoxTheatric, dzl_box_theatric, G_TYPE_OBJECT)

static GParamSpec *properties[LAST_PROP];

static void
get_toplevel_rect (DzlBoxTheatric *theatric,
                   GdkRectangle  *area)
{
                  ...
                  DzlBoxTheatric *self)
{
  ...
  g_assert (DZL_IS_BOX_THEATRIC (self));
  ...
  if (self->background_set)
    {
      ...
      dzl_cairo_rounded_rectangle (cr, &area, 3, 3);
...
static void
dzl_box_theatric_notify (GObject    *object,
                        GParamSpec *pspec)
{
  DzlBoxTheatric *self = DZL_BOX_THEATRIC (object);

  if (G_OBJECT_CLASS (dzl_box_theatric_parent_class)->notify)
    G_OBJECT_CLASS (dzl_box_theatric_parent_class)->notify (object, pspec);
  ...
  if (self->target && self->toplevel)
    {
      ...
      get_toplevel_rect (DZL_BOX_THEATRIC (object), &area);
...
static void
dzl_box_theatric_dispose (GObject *object)
{
  DzlBoxTheatric *self =  DZL_BOX_THEATRIC (object);
...
static void
dzl_box_theatric_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
  DzlBoxTheatric *theatric = DZL_BOX_THEATRIC (object);
...
static void
dzl_box_theatric_set_property (GObject      *object,
                              guint         prop_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
...
static void
dzl_box_theatric_class_init (DzlBoxTheatricClass *klass)
{
  GObjectClass *object_class;

  object_class = G_OBJECT_CLASS (klass);
  object_class->dispose = dzl_box_theatric_dispose;
  object_class->notify = dzl_box_theatric_notify;
  object_class->get_property = dzl_box_theatric_get_property;
  object_class->set_property = dzl_box_theatric_set_property;

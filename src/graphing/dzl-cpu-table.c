...
G_DEFINE_TYPE (DzlCpuTable, dzl_cpu_table, DZL_TYPE_GRAPH_MODEL)

#ifdef __linux__
static void
dzl_cpu_table_poll (DzlCpuTable *self)
{
  gchar cpu[64] = { 0 };
  glong user;
  glong sys;
...
static gboolean
dzl_cpu_table_poll_cb (gpointer user_data)
{
  DzlCpuTable *self = user_data;
  DzlGraphModelIter iter;
  guint i;

  dzl_cpu_table_poll (self);
...
static void
dzl_cpu_table_constructed (GObject *object)
{
  DzlCpuTable *self = (DzlCpuTable *)object;
  gint64 timespan;
  guint max_samples;
  guint i;

  G_OBJECT_CLASS (dzl_cpu_table_parent_class)->constructed (object);
  ...
  self->poll_source = g_timeout_add (self->poll_interval_msec, dzl_cpu_table_poll_cb, self);
}

static void
dzl_cpu_table_finalize (GObject *object)
{
  DzlCpuTable *self = (DzlCpuTable *)object;
  ...
  G_OBJECT_CLASS (dzl_cpu_table_parent_class)->finalize (object);
}

static void
dzl_cpu_table_class_init (DzlCpuTableClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->constructed = dzl_cpu_table_constructed;
  object_class->finalize = dzl_cpu_table_finalize;
}

static void
dzl_cpu_table_init (DzlCpuTable *self)
{
